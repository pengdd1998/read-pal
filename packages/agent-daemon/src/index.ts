...
const PROJECT_ROOT = process.env.PROJECT_ROOT || '/Volumes/ExternalDisk/read-pal';
...
2. Run "curl -s http://192.168.1.13:3001/health" to check API health (or localhost:3001 if local)
...
4. SSH to 192.168.1.13 and deploy: pull, build API, build web, restart PM2
   Use key: ssh -i ~/.ssh/local_ubuntu_key ubuntu@192.168.1.13
5. Verify health with curl http://192.168.1.13:3001/health
