...
async def _enrich_with_synthesis_themes(
    db: AsyncSession,
    user_id: UUID,
    book_id: UUID,
) -> list[str]:
...
async def _enrich_reading_metrics(
    sessions: list[dict],
...
async def _collect_enriched_data(
    ...
    kg = await _enrich_with_knowledge_graph(db, user_id, book_id)
    enriched.update(kg)
    ...
    # Synthesis themes
    enriched['synthesis_themes'] = await _enrich_with_synthesis_themes(
        db, user_id, book_id,
    )

    # Reading pace, session details, and first highlight
    metrics = await _enrich_reading_metrics(
    ...
    books = await _fetch_other_books(db, user_id, book_id)
