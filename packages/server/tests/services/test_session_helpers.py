...
def test_finalize_duration_uses_existing_value_if_set():
    ...
    started = datetime.now(tz=UTC) - timedelta(seconds=600)
    session = ReadingSession(started_at=started, duration=120)
    finalize_session_duration(session, datetime.now(tz=UTC))
...
def test_finalize_duration_caps_idle_session_to_grace():
    ...
    """
    started = datetime.now(tz=UTC) - timedelta(seconds=86400)
...
def test_finalize_duration_caps_idle_tab_to_grace_window():
    ...
    """
    started = datetime.now(tz=UTC) - timedelta(hours=4)
...
def test_finalize_duration_uses_recent_heartbeat_when_present():
    ...
    started = datetime.now(tz=UTC) - timedelta(minutes=30)
    last_heartbeat = datetime.now(tz=UTC) - timedelta(minutes=20)
...
def test_clamp_client_duration_caps_idle_tab():
    ...
    clamped = clamp_client_duration(session, 14400, datetime.now(tz=UTC))
...
def test_clamp_client_duration_respects_max_session_seconds():
    ...
    started = datetime.now(tz=UTC) - timedelta(minutes=10)
    session = ReadingSession(started_at=started, updated_at=datetime.now(tz=UTC))
    # Client claims 3h reading
    clamped = clamp_client_duration(session, 10800, datetime.now(tz=UTC))
...
def test_clamp_client_duration_uses_smaller_of_reported_and_wall():
    ...
    clamped = clamp_client_duration(session, 120, datetime.now(tz=UTC))
...
def test_clamp_client_duration_without_started_at_only_caps_max():
    ...
    clamped = clamp_client_duration(session, 99999, datetime.now(tz=UTC))
