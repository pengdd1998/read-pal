...
async def test_reset_password_invalid_token(client):
    ...
    resp = await client.post(
        ...
        json={'token': 'invalid-token', 'password': fake_password('NewPass', '123')},
    )
...
async def test_reset_password_valid_token(client):
    ...
    reg = await register_user(client, email='reset@test.com', password=fake_password('OldPass', '123'))
    user_id = reg['user']['id']
    ...
    with (
        ...
        resp = await client.post(
            '/api/v1/auth/reset-password',
            json={'token': token, 'password': fake_password('NewPass', '456')},
        )
        assert resp.status_code == 200
...
async def test_reset_password_then_login(client):
    ...
    reg = await register_user(client, email='loginreset@test.com', password=fake_password('OldPass', '123'))
    user_id = reg['user']['id']
    ...
    with (
        ...
        resp = await client.post(
            '/api/v1/auth/reset-password',
            json={'token': token, 'password': fake_password('BrandNew', '789')},
        )
        assert resp.status_code == 200
    ...
    resp = await client.post(
        '/api/v1/auth/login',
        json={'email': 'loginreset@test.com', 'password': fake_password('BrandNew', '789')},
    )
    assert resp.status_code == 200
    ...
    resp = await client.post(
        '/api/v1/auth/login',
        json={'email': 'loginreset@test.com', 'password': fake_password('OldPass', '123')},
    )
...
async def test_reset_password_token_consumed_atomically(client):
    ...
    """
    reg = await register_user(client, email='race@test.com', password=fake_password('OldPass', '123'))
    user_id = reg['user']['id']
    ...
    with (
        ...
        first = await client.post(
            '/api/v1/auth/reset-password',
            json={'token': token, 'password': fake_password('FirstNew', '456')},
        )
        second = await client.post(
            '/api/v1/auth/reset-password',
            json={'token': token, 'password': fake_password('SecondNew', '789')},
        )
