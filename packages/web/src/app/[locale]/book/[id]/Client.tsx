    ...
    try {
      ...
      canvas.toBlob(async (blob) => {
        ...
        if (navigator.share && navigator.canShare?.({ files: [file] })) {
          try {
            await navigator.share({ files: [file], title: `${bookTitle} — read-pal`, text: `"${text}" — ${bookAuthor || ''}` });
          } catch (err) {
...
export default function BookDetailPage() {
  ...
  return (
    <main className="max-w-2xl mx-auto px-4 sm:px-6 py-12 animate-fade-in">
      ...
      {/* Progress */}
      <div className="bg-surface-0 rounded-2xl border border-gray-200 dark:border-gray-800 p-6 mb-6 animate-slide-up stagger-2">
      ...
      {allAnnotations.length > 0 ? (
        <div className="bg-surface-0 rounded-2xl border border-gray-200 dark:border-gray-800 mb-6 animate-slide-up stagger-3 overflow-hidden">
      ...
      {totalAnnotations > 0 && (
        <div className="flex flex-wrap gap-2 mb-6 animate-slide-up stagger-4">
          <button
            ...
            className="inline-flex items-center gap-1.5 px-3 sm:px-4 py-2 rounded-xl text-xs sm:text-sm font-medium bg-surface-0 border border-gray-200 dark:border-gray-700 text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors"
          >
            <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
      ...
      {allAnnotations.filter((a) => a.type === 'highlight').length > 0 && (
        <ShareQuoteSection
          highlights={allAnnotations.filter((a) => a.type === 'highlight').slice(0, 5)}
      ...
      <div className="flex gap-3 animate-slide-up stagger-4">
        ...
        <Link
          href="/library"
          className="btn bg-surface-0 border border-gray-200 dark:border-gray-700 text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700"
        >
          {t('library')}
        </Link>
      </div>
