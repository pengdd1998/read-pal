    ...
    <main className="max-w-3xl mx-auto px-4 sm:px-6 py-8 sm:py-12 animate-fade-in">
      ...
      {loading ? (
        ...
        <div className="text-center py-16 animate-fade-in">
          ...
          <h2 className="text-lg font-semibold text-gray-900 dark:text-white mb-2">{t('no_stats_title')}</h2>
        ...
        <div className="space-y-5">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {[
              ...
              { label: t('label_streak'), value: `${stats?.readingStreak || 0}d`, icon: '🔥', color: 'text-orange-600 dark:text-orange-400', bg: 'bg-orange-50 dark:bg-orange-900/10' },
              { label: t('label_time'), value: stats?.totalTime || formatTime(Math.round(totalMinutes / 60)), icon: '⏱️', color: 'text-violet-600 dark:text-violet-400', bg: 'bg-violet-50 dark:bg-violet-900/10' },
            ].map((item, i) => (
          ...
          <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 p-6">
            ...
            <div className="space-y-3">
              {[
                { label: t('status_reading'), count: statusCounts.reading, color: 'bg-amber-500', pct: stats?.booksRead ? (statusCounts.reading / (statusCounts.reading + statusCounts.unread + statusCounts.completed)) * 100 : 0 },
                { label: t('status_completed'), count: statusCounts.completed, color: 'bg-emerald-500', pct: stats?.booksRead ? (statusCounts.completed / (statusCounts.reading + statusCounts.unread + statusCounts.completed)) * 100 : 0 },
                { label: t('status_unread'), count: statusCounts.unread, color: 'bg-gray-300 dark:bg-gray-600', pct: stats?.booksRead ? (statusCounts.unread / (statusCounts.reading + statusCounts.unread + statusCounts.completed)) * 100 : 0 },
              ].map((item) => (
