export default function MemoryBooksLoading() {
 ...
 <div className="px-4 sm:px-6 lg:px-8 py-8 sm:py-12 animate-fade-in">
  <div className="mb-8">
  <div className="h-8 bg-surface-2 rounded-lg w-44 animate-pulse" />
  ...
  {Array.from({ length: 4 }).map((_, i) => (
   ...
   <div className="flex items-center gap-3 mb-3">
    <div className="w-12 h-16 rounded-lg bg-surface-2" />
   ...
   <div className="grid grid-cols-3 gap-2">
    ...
    <div key={j} className="h-12 bg-surface-1 rounded-lg" />
    ))}
   </div>
   </div>
  ))}
