...
export default function LibraryPage() {
  ...
  useEffect(() => {
    api.get<{ books: FreeBook[] }>('/api/discovery/free-books')
      ...
      .catch(() => { toast(t('toast_suggestions_fail'), 'error'); })
  ...
  const handleSeedSample = async () => {
    ...
    try {
      ...
      if (res.success && res.data) {
        ...
        toast(res.error?.message || t('toast_add_book_fail'), 'error');
      }
    } catch {
      ...
      {!loadingSuggestions && filteredSuggestions.length > 0 && (
        <div className="mt-12 pt-8 border-t border-gray-100 dark:border-gray-800">
          <div className="flex items-center justify-between mb-5">
            <div>
              <h2 className="text-lg font-bold text-gray-900 dark:text-white mb-1">{t('free_books_title')}</h2>
