...
const RATINGS: { value: number; labelKey: string; hint: string; color: string }[] = [
 ...
 { value: 3, labelKey: 'rating_hard', hint: '2', color: 'bg-orange-100 text-orange-700 dark:bg-orange-900/30 dark:text-orange-300' },
...
export function FlashcardStudy({
 ...
 <main className="max-w-lg mx-auto px-4 py-12 animate-fade-in">
  ...
  <button
   ...
   className="inline-flex items-center gap-2 px-3 py-1.5 rounded-lg text-sm text-gray-500 hover:text-amber-600 hover:bg-amber-50 dark:hover:bg-amber-900/20 transition-colors min-h-[44px]"
  >
  ...
  <div className="animate-slide-up">
   <p className="text-xs text-gray-400 text-center mb-3">{t('how_well')} <span className="hidden sm:inline text-gray-400">{t('keys_hint')}</span></p>
   ...
   {RATINGS.map(({ value, labelKey, hint, color }) => (
    ...
    disabled={reviewing}
    className={`flex flex-col items-center gap-0.5 px-1 py-3 rounded-xl text-xs font-semibold transition-all duration-200 hover:scale-105 active:scale-95 disabled:opacity-50 min-h-[44px] ${color}`}
    >
