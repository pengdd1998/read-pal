 ...
 <section className="bg-surface-1 py-20">
  ...
  <div className="grid sm:grid-cols-3 gap-8">
   ...
   <div key={persona.title} className="card-hover text-center">
    ...
    <p className="text-sm text-gray-600 leading-relaxed mb-4">{persona.desc}</p>
    <ul className="space-y-1.5">
    {persona.features.map((f) => (
     <li key={f} className="text-xs text-gray-500 flex items-center justify-center gap-1.5">
     <CheckIcon />
     {f}
     </li>
    ))}
