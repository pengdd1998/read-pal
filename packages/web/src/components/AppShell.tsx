 ...
 <div className="min-h-screen flex flex-col bg-surface-1">
  ...
  <div className="px-4 sm:px-6 lg:px-8">
   ...
   <div className="flex items-center gap-1 sm:gap-3 min-w-0">
    {mounted && isAuthenticated ? (
    <>
     <Link href="/search" prefetch={false} className="hidden md:min-w-[44px] md:min-h-[44px] md:flex items-center justify-center rounded-lg text-gray-500 hover:text-amber-600 dark:hover:text-amber-400 hover:bg-amber-50 dark:hover:bg-amber-900/20 transition-colors" aria-label={t('nav_search')}>
     ...
     <button type="button" onClick={logout} className="hidden md:inline-flex btn btn-ghost text-xs sm:text-sm text-gray-500 hover:text-primary-700 dark:hover:text-white focus-visible:ring-2 focus-visible:ring-amber-500">{tc('logout')}</button>
    </>
    ) : (
    <Link href="/auth?mode=login" className="btn btn-secondary text-sm">{tc('login')}</Link>
    )}
    {mounted && isAuthenticated && navMode === 'mobile' && (
    <button type="button" onClick={() => setMobileOpen(!mobileOpen)} className="min-w-[44px] min-h-[44px] flex items-center justify-center rounded-lg text-gray-500 hover:bg-surface-2 transition-colors focus-visible:ring-2 focus-visible:ring-amber-500 focus-visible:ring-offset-1"
  ...
  {mounted && isAuthenticated && mobileOpen && navMode === 'mobile' && (
   ...
   <div className="px-4 py-3 space-y-1">
    ...
    <LanguageSwitcher />
    <button type="button" onClick={() => { handleMobileNav(); logout(); }} className="px-3 py-2 rounded-lg text-sm text-gray-500 hover:bg-surface-2/60 transition-colors focus-visible:ring-2 focus-visible:ring-amber-500 min-h-[44px]">
     {tc('logout')}
    </button>
    </div>
   </div>
