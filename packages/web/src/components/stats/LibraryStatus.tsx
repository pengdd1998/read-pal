...
export function LibraryStatus({ stats, statusCounts }: LibraryStatusProps) {
 ...
 { label: t('status_unread'), count: statusCounts.unread, color: 'bg-gray-300', pct: stats?.booksRead ? (statusCounts.unread / total) * 100 : 0 },
 ], [stats, statusCounts, total, t]);

 return (
 <div className="bg-surface-0 rounded-xl border border-surface-3 p-6">
  <h2 className="font-semibold text-gray-900 mb-4">{t('library_status')}</h2>
