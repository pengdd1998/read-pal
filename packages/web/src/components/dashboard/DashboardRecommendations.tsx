...
export const DashboardRecommendations = memo(function DashboardRecommendations() {
 ...
 <div className="card">
  ...
  <Link href="/search" prefetch={false} className="text-[10px] text-primary-600 dark:text-primary-400 hover:underline focus-visible:ring-2 focus-visible:ring-amber-400 focus-visible:outline-none">{t('see_all')}</Link>
  </div>
  <div className="space-y-2">
  {topRecs.map((r) => (
