...
export function ReadingGoals() {
 ...
 return (
  ...
  <div className="flex justify-center gap-10">
   ...
   <div key={i} className="flex flex-col items-center gap-3">
    <div className="w-28 h-28 bg-gray-100 rounded-full animate-pulse" />
 ...
 return (
  ...
  <div className="flex items-center gap-2 mb-4">
   ...
   <h3 className="font-bold text-gray-900">{t('goals_title')}</h3>
  </div>
  <p className="text-sm text-gray-500">{error ?? t('goals_no_data')}</p>
  {error && (
   <button onClick={fetchGoals} className="mt-2 text-xs text-amber-600 dark:text-amber-400 hover:underline min-h-[44px] inline-flex items-center">
    Retry
   </button>
  )}
  </div>
 ...
 <div className="rounded-2xl border border-surface-2 bg-surface-0 p-6">
  ...
  <div className="flex flex-col items-center gap-2">
   ...
   <div className="text-center">
    <span className="text-lg font-bold text-gray-900 tabular-nums">{data.todayMinutes}</span>
   ...
   {dailyComplete
    ...
    : <span className="text-gray-500">{t('goals_min_to_go', { count: data.dailyRemaining })}</span>}
   </p>
  ...
  <div className="flex flex-col items-center gap-2">
   ...
   <div className="text-center">
    <span className="text-lg font-bold text-gray-900 tabular-nums">{data.completed}</span>
   ...
   {weeklyComplete
    ? <span className="text-emerald-600 dark:text-emerald-400">{t('goals_reached')}</span>
    : <span className="text-gray-500">{t('goals_books_to_go', { count: data.remaining })}</span>}
   </p>
  ...
  {!editingGoal ? (
   ...
   className="flex items-center gap-1.5 text-xs text-gray-400 hover:text-primary-600 dark:hover:text-primary-400 transition-colors mx-auto min-h-[44px] px-3">
   ...
   <p className="text-xs font-medium text-gray-600">{t('goals_daily_reading_goal')}</p>
   <div className="flex items-center gap-3">
    <button onClick={() => adjustMinutes(-5)} disabled={pendingMinutes <= 10 || saving}
    aria-label={t('goals_decrease_aria')}
    className="w-11 h-11 rounded-lg bg-gray-100 text-gray-600 flex items-center justify-center hover:bg-gray-200 disabled:opacity-40 disabled:cursor-not-allowed transition-colors focus-visible:ring-2 focus-visible:ring-amber-400 focus-visible:ring-offset-1">
    ...
    <span className="text-lg font-bold text-gray-900 tabular-nums w-14 text-center">{pendingMinutes}</span>
   ...
   <div className="flex items-center gap-2">
    ...
    className="px-3 py-1.5 text-xs font-medium rounded-lg bg-primary-600 text-white hover:bg-primary-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors min-h-[44px]">
    {saving ? t('goals_saving') : t('goals_save')}
    </button>
    <button onClick={() => setEditingGoal(false)} disabled={saving}
    className="px-3 py-1.5 text-xs font-medium rounded-lg bg-gray-100 text-gray-600 hover:bg-gray-200 disabled:opacity-50 disabled:cursor-not-allowed transition-colors min-h-[44px]">
    {t('goals_cancel')}
    </button>
   </div>
