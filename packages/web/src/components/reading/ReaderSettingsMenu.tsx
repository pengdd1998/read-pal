...
export const ReaderSettingsMenu = React.memo(function ReaderSettingsMenu(props: ReaderSettingsMenuProps) {
  ...
  // Dismiss on outside-click (excluding the header toolbar — its buttons toggle
  // this menu) and on Escape. Deliberately NOT a full-screen blocking overlay:
  // a reading-settings popover needs the underlying text to stay visible and
  // scrollable so the reader can preview font/theme/line-height changes live.
  useEffect(() => {
    if (!show) return;
    const onMouseDown = (e: MouseEvent) => {
      ...
      if (target.closest('[data-reader-header]')) return;
      onClose();
    };
