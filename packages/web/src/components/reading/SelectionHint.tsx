...
export const SelectionHint = React.memo(function SelectionHint({ onDismiss }: SelectionHintProps) {
 ...
 <div className="fixed bottom-28 left-1/2 -translate-x-1/2 z-20 animate-fade-in" role="status" aria-live="polite" style={{ animation: 'fade-in 0.5s 1.5s forwards' }}>
  ...
  <button onClick={onDismiss} className="ml-1 opacity-60 hover:opacity-100 transition-opacity focus-visible:ring-2 focus-visible:ring-white focus-visible:ring-offset-2 min-w-[44px] min-h-[44px] flex items-center justify-center" aria-label={t('dismiss_label')}>
