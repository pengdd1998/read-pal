...
export const PlanLoadingSkeleton = React.memo(function PlanLoadingSkeleton() {
  return (
    <div className="space-y-3" aria-busy="true" aria-label="Loading reading plan">
      <div className="h-4 skeleton rounded animate-pulse" />
      <div className="h-4 skeleton rounded w-3/4 animate-pulse" />
      <div className="h-4 skeleton rounded w-1/2 animate-pulse" />
    </div>
  );
});
...
export const PlanGenerateForm = React.memo(function PlanGenerateForm({
  ...
  return (
    <div className="space-y-4">
      {error && (
        <div role="alert" className="p-3 rounded-lg bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800/30 text-xs text-red-700 dark:text-red-300 flex items-center justify-between">
          <span>{error}</span>
          <button
            type="button"
            onClick={onGenerate}
      ...
      <div>
        ...
        <input
          ...
          max={90}
          inputMode="numeric"
          aria-label={t('reading_plan_total_days')}
      ...
      <div>
        ...
        <input
          ...
          max={240}
          inputMode="numeric"
          aria-label={t('reading_plan_daily_minutes')}
          className="w-full mt-1 px-3 py-2 text-sm border border-surface-3 rounded-lg bg-surface-0 text-gray-800 dark:text-gray-200 focus:ring-2 focus:ring-amber-500 focus:border-transparent outline-none"
        />
      </div>
