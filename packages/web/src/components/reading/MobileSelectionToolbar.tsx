 ...
 >
  ...
  <div className="flex items-center justify-around px-4 py-3">
   ...
   className={`flex flex-col items-center gap-1 active:scale-95 transition-transform ${
    showTagPicker ? 'text-amber-600 dark:text-amber-400' : 'text-gray-600'
   }`}
   aria-label={t('toolbar_tag_and_highlight')}
   >
   <div className={`w-11 h-11 rounded-xl flex items-center justify-center ${
    showTagPicker ? 'bg-amber-100 dark:bg-amber-900/30' : 'bg-gray-100'
   }`}>
    <svg aria-hidden="true" className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
   ...
   >
   <div className={`w-11 h-11 rounded-xl flex items-center justify-center ${copied ? 'bg-emerald-100 dark:bg-emerald-900/30' : 'bg-gray-100'}`}>
    {copied ? (
