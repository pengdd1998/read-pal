...
const ToastItem = React.memo(function ToastItem({ toast, onDismiss }: { toast: Toast; onDismiss: (id: string) => void }) {
 ...
 >
  ...
  <button type="button" onClick={() => { setVisible(false); setTimeout(() => onDismiss(toast.id), 300); }} aria-label={tc('toast_dismiss_aria')} className="text-current opacity-50 hover:opacity-100 transition-opacity flex-shrink-0 min-h-[44px] min-w-[44px] inline-flex items-center justify-center focus-visible:ring-2 focus-visible:ring-amber-500 focus-visible:ring-offset-1">
