...
export function CollectionCreateForm({
 ...
 <div className="mb-3 p-3 bg-surface-0 border border-surface-3 rounded-xl space-y-2 animate-slide-up">
  ...
  className="w-full px-2.5 py-1.5 text-sm bg-gray-50 border border-surface-3 rounded-lg outline-none focus:ring-1 focus:ring-primary-400/50"
  autoFocus
  />
  {/* Icon picker */}
  ...
  {icons.map((ic) => (
   ...
   className={`p-1.5 rounded-md transition-colors focus-visible:ring-2 focus-visible:ring-amber-400 ${newIcon === ic.value ? 'bg-primary-100 dark:bg-primary-900/30 text-primary-600' : 'text-gray-400 hover:text-gray-600 hover:bg-gray-100'}`}
   title={ic.label}
   >
  ...
  {DEFAULT_COLORS.map((c) => (
   ...
   className={`w-5 h-5 rounded-full transition-transform focus-visible:ring-2 focus-visible:ring-amber-400 ${newColor === c ? 'scale-125 ring-2 ring-offset-1 ring-gray-300' : 'hover:scale-110'}`}
