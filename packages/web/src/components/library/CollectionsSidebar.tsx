...
export const CollectionsSidebar = React.memo(function CollectionsSidebar({ activeCollectionId, onSelectCollection }: CollectionsSidebarProps) {
 ...
 return (
  ...
  <div className="h-5 w-24 skeleton rounded animate-pulse" />
  {[1, 2, 3].map((i) => (
   <div key={i} className="h-8 skeleton rounded-lg animate-pulse" />
  ))}
  </div>
 );
 }
 ...
 return (
  ...
  <p role="alert" className="text-xs text-red-500 dark:text-red-400 mb-2">{t('collections_load_failed')}</p>
  <button type="button"
