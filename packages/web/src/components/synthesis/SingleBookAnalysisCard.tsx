...
export const SingleBookAnalysisCard = React.memo(function SingleBookAnalysisCard({ books, booksLoading, booksError }: SingleBookAnalysisCardProps) {
 ...
 <>
  ...
  <div role="alert" className="mb-6 p-4 rounded-xl border border-red-200 dark:border-red-800 bg-red-50 dark:bg-red-900/20 text-sm text-red-700 dark:text-red-300 flex items-center justify-between">
   <span>{error}</span>
   <button type="button" onClick={handleAnalyze} className="text-xs font-medium underline hover:no-underline focus-visible:ring-2 focus-visible:ring-amber-400 focus-visible:outline-none min-h-[44px] inline-flex items-center px-2">{t("retry")}</button>
  </div>
  )}
