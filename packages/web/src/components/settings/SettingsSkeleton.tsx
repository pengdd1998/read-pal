      ...
      {Array.from({ length: 3 }).map((_, i) => (
        <div key={i} className="mb-6">
          <div className="flex items-center gap-3 mb-4">
            ...
            <div className="h-5 skeleton rounded-lg w-32 animate-pulse" />
          </div>
          <div className="bg-surface-0 rounded-2xl border border-surface-3 p-6 space-y-5">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
              {Array.from({ length: 3 }).map((_, j) => (
                <div key={j} className="h-10 skeleton rounded-xl animate-pulse" />
              ))}
            </div>
            <div className="h-4 skeleton rounded w-20 animate-pulse" />
            <div className="h-2 skeleton rounded-full animate-pulse" />
          </div>
        </div>
      ))}
    </div>
