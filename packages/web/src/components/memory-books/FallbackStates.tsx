...
export const EmptyCta = React.memo(function EmptyCta({ onGenerate }: EmptyCtaProps) {
 ...
 <div className="max-w-md mx-auto px-4 py-20 text-center animate-fade-in">
  ...
  <p className="text-sm text-gray-500 dark:text-gray-400 mb-8 max-w-sm mx-auto">{t('yourPersonalBookDesc')}</p>
  <button type="button"
  onClick={onGenerate}
  ...
  <div className="mt-6">
  <Link href="/memory-books" prefetch={false} className="text-sm text-gray-500 dark:text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 transition-colors focus-visible:ring-2 focus-visible:ring-amber-400 focus-visible:outline-none min-h-[44px] inline-flex items-center">
   {t('backToMemoryBooks')}
  </Link>
  </div>
 </div>
