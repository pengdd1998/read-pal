...
export function ErrorState({ error, onRetry }: ErrorStateProps) {
 ...
 <div className="max-w-md mx-auto px-4 py-20 text-center animate-fade-in">
  ...
  <Link href="/memory-books" className="btn bg-gray-100 text-gray-700">
   {t('backToMemoryBooks')}
  </Link>
  </div>
 </div>
...
export function EmptyCta({ onGenerate }: EmptyCtaProps) {
 ...
 <div className="max-w-md mx-auto px-4 py-20 text-center animate-fade-in">
  ...
  <div className="mt-6">
  <Link href="/memory-books" className="text-sm text-gray-400 hover:text-gray-600 transition-colors">
   {t('backToMemoryBooks')}
  </Link>
  </div>
 </div>
