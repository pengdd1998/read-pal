...
function loadSavedPosition(storageKey: string): DragPosition | null {
  ...
  } catch (err) { warn('Storage error: failed to load draggable position', err); }
  return null;
}
...
export function useDraggable(options: UseDraggableOptions): UseDraggableReturn {
  ...
  useEffect(() => {
    try { safeSetItem(storageKey, JSON.stringify(btnPos)); } catch (err) { warn('Storage error: failed to persist draggable position', err); }
  }, [btnPos, storageKey]);
