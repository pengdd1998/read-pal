...
## Health Checks
1. **API Health**: `GET http://192.168.1.13:3001/health` — expect `{"status":"ok"}`
2. **Detailed Health**: `GET http://192.168.1.13:3001/health/detailed` — all services ok
3. **Web App**: `GET http://192.168.1.13:3000` — expect 200
4. **PM2 Status**: `ssh ubuntu@192.168.1.13 "pm2 jlist"` — all processes online
5. **Disk Space**: `ssh ubuntu@192.168.1.13 "df -h /"` — warn if > 80%
6. **Memory**: Check for memory leaks via PM2 monit
7. **Recent Logs**: `ssh ubuntu@192.168.1.13 "pm2 logs --lines 20 --nostream"` — check for errors
