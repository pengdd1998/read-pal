...
### Step 4: Deploy Flow
```bash
ssh -i ~/.ssh/REDACTED_KEY -o StrictHostKeyChecking=no ubuntu@REDACTED_IP bash -s <<'REMOTE'
set -e
...
cp -r REDACTED_DEPLOY_PATH/packages/web/public REDACTED_DEPLOY_PATH/packages/web/.next/standalone/packages/web/public 2>/dev/null || true
cp -r REDACTED_DEPLOY_PATH/packages/web/.next/static REDACTED_DEPLOY_PATH/packages/web/.next/standalone/packages/web/.next/static 2>/dev/null || true

pm2 delete all 2>/dev/null || true
sleep 1
pm2 start REDACTED_DEPLOY_PATH/packages/api/dist/index.js --name read-pal-api --cwd REDACTED_DEPLOY_PATH/packages/api
sleep 2
pm2 start node --name read-pal-web --cwd REDACTED_DEPLOY_PATH/packages/web -- .next/standalone/packages/web/server.js
pm2 save
sleep 5
